...
| 能力 | 说明 |
|------|------|
| 麦克风 / 内录 / 文件 | 麦克风；内部音频（API 29+，MediaProjection + 多采样率回退）；**本地文件**（FFmpeg 转 16k WAV → 原 VAD/ASR/LLM 管线） |
...
| 历史 | 搜索、分段浏览、Markdown / SRT / 纯文本导出、录音分享 |
...
3. 主界面选 **麦克风** / **内部音频** / **文件** → **Start**
   ...
   - 内部音频：系统投屏授权 + 前台服务
   - 文件：系统文件选择器（mp3/m4a/wav/…）→ FFmpeg 转码 → 自动切句翻译（无需麦克风）
   - 悬浮字幕：需「显示在其他应用上层」
